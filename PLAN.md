...
**עלות**: **0 ש"ח בפועל** - שימוש משתמש יחיד עמוק בתוך ה-Tier החינמי של Firebase.

**אסטרטגיית חיוב**: Blaze Plan עם **מגבלת תקציב קשיחה של $5/חודש**. למה לא Spark?
- Spark עוצר את השירות כשחורגים, מה שעלול לעצור עבודה קריטית
- Blaze עם Cap נותן גמישות לחרוג קצת (במחיר אגורות) ובו-זמנית הגנה אבסולוטית מחשבון מפתיע
- בפועל, השימוש האמיתי יישאר ב-$0 גם בחודש כבד
...
- [ ] שדרוג ל-**Blaze Plan** וקביעת **Budget Alert ב-$5/חודש** עם Auto-Pause
  - Google Cloud Console → Billing → Budgets & alerts → Create Budget
  - Amount: $5 USD, Threshold: 50%, 90%, 100%
  - Action at 100%: Disable billing (השירות עוצר אוטומטית)
- [ ] אימות שההגדרה נכונה: מעבר לדף Billing ובדיקה שהמגבלה פעילה
